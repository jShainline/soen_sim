...
_temp_str_2 = 'dend_3jj__flux_drive_arrays__Ib{:06.2f}uA'.format(Ib)
...
Phi_ex_array = data_array_imported['Phi_ex_array']
...
num_steps = 25 # for excitatory flux

max_flux = p['Phi0__pH_ns']/2
_fr = max_flux/num_steps # flux_resolution
_fr_0 = max_flux/100 # flux_resolution of first onset point
...
if method == 'j_di_phase' or method == 'I_di':        
    ...
    polynomial_order = 3 # for savitzky-golay filter
...
if run_all == True:
    for qq in range(num_Phi_p): # [0]: # [3]: #        
        ...
        Phi_ex_vec = Phi_ex_array[qq]
        j_di_rate_array = []
        j_di_ifi_array = []            
        I_di_array = []
        
        num_drives = len(Phi_ex_vec)
        
        # load wr data, find peaks, find rates
        for ii in range(num_drives): # [0]: #
            Phi_ex = Phi_ex_vec[ii] 
            # print(Phi_a)
            Iex = Phi_ex/Mex
            # print(Iex)
            
            print('qq = {:d} of {:d} (Phi_p/Phi_0 = {:07.5f}); ii = {:d} of {:d} (Phi_ex/Phi0 = {:07.5f})'.format(qq+1,num_Phi_p,Phi_p/Phi0,ii+1,num_drives,Phi_ex/Phi0))
            
            directory = 'wrspice_data/3jj'          
            file_name = 'dend_3jj_nest_cnst_drv_seek_rt_arry_Ib{:06.2f}uA_Lp{:05.2f}pH_Ip{:09.6f}uA_Iex{:09.6f}.dat'.format(Ib,Lp,Ip,Iex)
            j_di_str = 'v(2)'
            j_di_phase_str = 'v(13)'
            I_di_str = 'i(L1)'
            ...
            if method == 'j_di_phase' or method == 'I_di':
                ...
                if (window_size__temp % 2) == 0:
                    window_size__temp -= 1
                poly_order__temp = np.min([window_size__temp-1,polynomial_order])
                I_di__avg = savgol_filter(I_di__avg, window_size__temp, poly_order__temp) # polynomial order 3
                j_di_phase__avg = savgol_filter(j_di_phase__avg, window_size__temp, poly_order__temp) # polynomial order 3
                time_vec__avg = savgol_filter(time_vec__avg, np.min([window_size__temp,len(time_vec__avg)]), poly_order__temp) # polynomial order 3
        ...
        if plot_rate_arrays == True:
            ...
            plot_dend_rate_array__norm_to_phi0(I_di_array = I_di_array__scaled, influx_list = Phi_ex_vec, master_rate_array = master_rate_array, Ib = Ib, Phi_p = Phi_p)
        ...
        # save data
        save_string = 'master_dnd_rate_array_3jj_Ib{:06.2f}uA_Lp{:05.2f}pH_Ip{:09.6f}uA_Ldi{:07.2f}nH_dt{:04.1f}ps_dsf{:d}'.format(Ib,Lp,Ip,Ldi,dt,downsample_factor)
        data_array = dict()
