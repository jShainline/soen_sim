...
for ii in range(len(Ib_vec)):
    ...
    for jj in range(len(Ip_vec)):
        ...
        file_name = 'dend_4jj_one_bias_plstc_lin_ramp_Ib{:05.2f}uA_Ip{:05.2f}uA.dat'.format(Ib,Ip)
        ...
        if len(j_sq_peaks) > 1:
            ...
            Phi_a_on__list.append(Ma*Ia[j_sq_peaks[0]])
                        
    Phi_a_on__array.append(np.asarray(Phi_a_on__list))
    Phi_p__array.append(np.asarray(Phi_p__list))
    
#%% plot

fig, ax = plt.subplots(nrows = 1, ncols = 1)
# fig.suptitle('Ib = {:6.2f}uA, Ip = {:7.4f}uA'.format(Ib,Ip))
color_list = ['blue1','blue3','blue5','green1','green3','green5','yellow3']
for ii in range(len(Ib_vec)):
    ax.plot(Phi_p__array[ii]/Phi0,Phi_a_on__array[ii]/Phi0, color = colors[color_list[ii]], label = 'Ib = {}uA'.format(Ib_vec[ii]))
ax.set_xlim([-1/2,1/2])
ax.set_ylim([0,1/2])
...
for jj in range(len(Ib_vec)):
    for ii in range(len(Phi_a_on__array[jj])):
        _ts = '{}{:f},'.format(_ts,Phi_a_on__array[jj][ii])    
    _ts = '{}],['.format(_ts[0:-1])
print('{}]'.format(_ts[0:-2]))

_ts = 'Phi_p__array = [['
for jj in range(len(Ib_vec)):
    for ii in range(len(Phi_p__array[jj])):
        _ts = '{}{:f},'.format(_ts,Phi_p__array[jj][ii])    
    _ts = '{}],['.format(_ts[0:-1])
print('{}]'.format(_ts[0:-2]))
