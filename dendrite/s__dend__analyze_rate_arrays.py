...
for mm in [2]: # range(num_Ib):
    ...
    for ii in np.arange(2,num_Phi_p-2): # range(num_Phi_p):
        ...
        file_name = 'master_dnd_rate_array_4jj_Ib{:06.2f}uA_Ip{:09.6f}uA_Ldi0077.50nH_dt01.0ps_dsf200.soen'.format(Ib,Ip)
        ...
        if do__squid_response == True or do__squid_threshold_composite == True:
            ...
            if do__squid_response == True :
                ...
                ax_sq.plot(influx_list/p['Phi0__pH_ns'],np.asarray(rate_vec__temp), '-', label = 'Ib = {:2.0f}uA, Ip = {:9.6f}uA'.format(Ib,Ip)) # , label = legend_text
            if do__squid_threshold_composite == True:
