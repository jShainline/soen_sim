...
def plot_neuronal_response__single_synaptic_pulse__wr_compare(neuron_instance,wr_instance):
    ...
    if neuron_instance.num_jjs == 4:
        I_spd__wr = np.asarray(wr_instance['L9#branch'])
        I_si__wr = np.asarray(wr_instance['L6#branch'])
        I_ni__wr = np.asarray(wr_instance['L2#branch'])
        I_ri__wr = np.asarray(wr_instance['L14#branch'])
...
def plot_neuronal_response__single_synaptic_pulse__no_rd__wr_compare(neuron_instance,wr_instance):
    ...
    # neuron
    axs[2].plot(time_vec*1e6,neuron_instance.influx_vec/Phi_0, '-', color = colors['blue3'], label = neuron_instance.name+': $n_j$ = '+'{:d}'.format(neuron_instance.num_jjs))
    nr_flux__wr = (1e6*I_si__wr*neuron_instance.synapses[name_1].M)/Phi_0
    axs[2].plot(time_vec__wr*1e6,nr_flux__wr, '-', color = colors['blue1'], label = 'wr')
...
def plot_neuronal_response__single_synaptic_pulse__no_rd__direct_feedback__wr_compare__just_one(neuron_instance,wr_instance,error):
    ...
    if legends == True:
        ...
        fig.suptitle('$n_j$ = {:d}; num in burst = {:d}; max rate = {:5.2f}MHz; ISI error = {:7.5e}'.format(neuron_instance.num_jjs,len(neuron_instance.spike_times),_term,error))
    ...
    y1 = neuron_instance.dendrites['{}__d'.format(neuron_instance.name)].influx_list__dend[1]/Phi_0
    ylims = axs.get_ylim()
    axs.plot([x1,x2],[y1,y1], ':', color = colors['redgrey5'], linewidth = pp['fine_linewidth'], label = 'ne__d thresh')
    axs.set_ylim(ylims)
    axs.set_ylabel(r'$\Phi^{nr}_{a}/\Phi_0$')
...
def plot_neuronal_response__single_synaptic_pulse__no_rd__direct_feedback__wr_compare__just_two(neuron_instance,wr_instance,error):
    ...
    axs[0].plot(time_vec,neuron_instance.influx_vec/Phi_0, '-', color = colors['blue3'], label = neuron_instance.name+'; $n_j$ = {:d};'.format(neuron_instance.num_jjs)+' $I_{ne}$ = '+'{:5.2f}uA'.format(neuron_instance.bias_currents[0]))
    # axs[0].plot(time_vec*1e6,neuron_instance.influx_vec__no_refraction/Phi_0, '-', color = colors['blue2'], label = neuron_instance.name+'; no ref')
    # axs[0].plot(time_vec*1e6,neuron_instance.influx_vec__just_refraction/Phi_0, '-', color = colors['blue4'], label = neuron_instance.name+'; just ref')
    nr_flux__wr = (I_si__wr*neuron_instance.synapses[name_1].M - I_threshold_2*neuron_instance.refraction_M)/Phi_0
    axs[0].plot(time_vec__wr,nr_flux__wr, '-', color = colors['red3'], label = 'wr; $M^{ni|si} = $'+'{:5.2f}pH'.format(neuron_instance.synapses[name_1].M))
    # axs[0].plot(time_vec__wr*1e6,1e6*I_si__wr*neuron_instance.synapses[name_1].M/Phi_0, '-', color = colors['red2'], label = 'wr; no ref')
    # axs[0].plot(time_vec__wr*1e6,1e18*I_threshold_2*neuron_instance.refraction_M/Phi_0, '-', color = colors['red4'], label = 'wr; just ref')
    ...
    # axs[1].plot(time_vec*1e6,1e6*neuron_instance.threshold_I2, '-', color = colors['green3'], label = neuron_instance.name+': threshold I2')
    axs[1].plot(time_vec,neuron_instance.threshold_I1, '-', color = colors['green3'], label = neuron_instance.name+': threshold I1')
    # axs[1].plot(time_vec__wr*1e6,I_threshold_2*1e6, '-', color = colors['yellow3'], label = 'wr: $I_{th2}$')
    axs[1].plot(time_vec__wr,I_threshold_1, '-', color = colors['yellow3'], label = 'wr: $I_{th1}$')
    axs[1].plot([time_vec[0],time_vec[-1]],neuron_instance.threshold_junction_critical_current*np.ones([2,1]), ':', color = colors['greengrey3'], linewidth = pp['fine_linewidth'], label = 'threshold')
    axs[1].set_ylabel(r'$I^{th}$ [$\mu$A]')
...
def plot_neuronal_response__single_synaptic_pulse__no_rd__direct_feedback__wr_compare__just_three(neuron_instance,wr_instance,error,I_nf_peaks__wr):
    ...
    for name_1 in neuron_instance.input_synaptic_connections:
        ...
        for _time in neuron_instance.synapses[name_1].input_spike_times:
            _time_ind = (np.abs(neuron_instance.time_vec-_time)).argmin() 
            axs[0].plot(time_vec[_time_ind],neuron_instance.influx_vec[_time_ind]/Phi_0, 'o', markersize = pp['nominal_markersize'], color = colors['blue5'], label = 'spike time; $tau_{si}$ = '+'{:4.0f}ns'.format(neuron_instance.synapses[name_1].integration_loop_time_constant))  
...
def plot_neuronal_response__single_synaptic_pulse__no_rd__direct_feedback__wr_compare__just_four(neuron_instance,wr_instance,error):
...
def plot_neuronal_response__single_synaptic_pulse__no_rd__direct_feedback__wr_compare(neuron_instance,wr_instance,error):
    ...
    if legends == True:
        _term = neuron_instance.max_rate*1e3
        fig.suptitle('num in burst = {:d}; max rate = {:5.2f}MHz; ISI error = {:7.5e}'.format(len(neuron_instance.spike_times),_term,error))
    
    # input synapses     
    for name_1 in neuron_instance.input_synaptic_connections:
           
        axs[0].plot(time_vec,neuron_instance.synapses[name_1].I_spd2_vec*neuron_instance.synapses[name_1].M_self/Phi_0, 
                    '-', color = colors['green3'], label = name_1+'; $n_j$ = {:d};'.format(neuron_instance.synapses[name_1].num_jjs)+' $I_{sy}$ = '+'{:5.2f}$\mu$A'.format(neuron_instance.synapses[name_1].bias_currents[0]))
        axs[0].plot(time_vec__wr,I_spd__wr*neuron_instance.synapses[name_1].M_self/Phi_0,'-.', color = colors['green1'], label = 'wr; $M^{sy}$ = '+'{:5.2f}pH'.format(neuron_instance.synapses[name_1].M_self))
        x1 = time_vec[0]
        x2 = time_vec[-1]
        y1 = neuron_instance.synapses['{}'.format(name_1)].influx_list__dend[1]/Phi_0
        ylims = axs[0].get_ylim()
        ...
        axs[1].plot(time_vec,neuron_instance.synapses[name_1].I_di_vec, '-', color = colors['green3'],
                    label = name_1+'; $L_{si}$ = '+'{:5.2f}nH'.format(neuron_instance.synapses[name_1].integration_loop_total_inductance*1e-3)+
                            '; $tau_{si}$ = '+'{:4.0f}ns'.format(neuron_instance.synapses[name_1].integration_loop_time_constant))
        axs[1].plot(time_vec__wr,I_si__wr,'-.', color = colors['green1'], label = 'wr')
                
        for _time in neuron_instance.synapses[name_1].input_spike_times:
            _time_ind = (np.abs(neuron_instance.time_vec-_time)).argmin()
            axs[0].plot(time_vec[_time_ind],neuron_instance.synapses[name_1].I_spd2_vec[_time_ind], 'o', markersize = pp['nominal_markersize'], color = colors['green5'])  
            axs[1].plot(time_vec[_time_ind],neuron_instance.synapses[name_1].I_di_vec[_time_ind], 'o', markersize = pp['nominal_markersize'], color = colors['green5'])  
            axs[2].plot(time_vec[_time_ind],neuron_instance.influx_vec[_time_ind]/Phi_0, 'o', markersize = pp['nominal_markersize'], color = colors['blue5'])
    ...
    y1 = neuron_instance.dendrites['{}__d'.format(neuron_instance.name)].influx_list__dend[1]/Phi_0
    ylims = axs[2].get_ylim()
    axs[2].plot([x1,x2],[y1,y1], ':', color = colors['redgrey5'], linewidth = pp['fine_linewidth'], label = 'ne__d thresh')
    axs[2].set_ylim(ylims)
    ...
    axs[4].plot(time_vec,neuron_instance.I_ni_vec*neuron_instance.threshold_M/Phi_0, '-', color = colors['yellow3'], label = neuron_instance.name+'; $M^{th}$ = '+'{:5.2f}pH'.format(neuron_instance.threshold_M))
    nt_flux__wr = (I_ni__wr*neuron_instance.threshold_M)/Phi_0
    axs[4].plot(time_vec__wr,nt_flux__wr, '-.', color = colors['yellow1'], label = 'wr')
    axs[4].set_ylabel(r'$\Phi^{th}_a/\Phi_0$')
    
    axs[5].plot(time_vec,neuron_instance.threshold_I2, '-', color = colors['green3'], label = neuron_instance.name+': threshold I2')
    axs[5].plot(time_vec,neuron_instance.threshold_I1, '-', color = colors['green1'], label = neuron_instance.name+': threshold I1')
    axs[5].plot(time_vec__wr,I_threshold_2, '-', color = colors['yellow3'], label = 'wr: $I_{th2}$')
    axs[5].plot(time_vec__wr,I_threshold_1, '-', color = colors['yellow1'], label = 'wr: $I_{th1}$')
...
def plot_wr_comparison__dend_drive_and_response(main_title,target_data__drive,actual_data__drive,target_data,actual_data,wr_data_file_name,error__drive,error__signal):
    ...
    M = np.sqrt(200*20)
    Phi0 = 6.62606957e3/(2*1.60217657)
    _pf = M/Phi0
    ...
    axs[0].plot(actual_data__drive[0,:],_pf*actual_data__drive[1,:], '-', color = colors['blue3'], linewidth = pp['nominal_linewidth'], markersize = pp['nominal_markersize'], label = 'soen_sim (error = {:7.5e})'.format(error__drive))   
    ...
    axs[0].plot(target_data__drive[0,0:tf_ind],_pf*target_data__drive[1,0:tf_ind], '-', color = colors['red3'], linewidth = pp['nominal_linewidth'], markersize = pp['nominal_markersize'], label = 'WRSpice')             
    # axs[0].set_xlabel(r'Time [$\mu$s]')
    ...
    axs[1].plot(actual_data[0,:],actual_data[1,:], '-', color = colors['blue3'], linewidth = pp['nominal_linewidth'], markersize = pp['nominal_markersize'], label = 'soen_sim (error = {:7.5e})'.format(error__signal))   
    axs[1].plot(target_data[0,0:tf_ind],target_data[1,0:tf_ind], '-', color = colors['red3'], linewidth = pp['nominal_linewidth'], markersize = pp['nominal_markersize'], label = 'WRSpice')             
    axs[1].set_xlabel(r'Time [ns]')
...
def plot_wr_comparison__dend_drive_and_response__compare_two(file_name,target_data__drive,actual_data__drive__1,actual_data__drive__2,target_data,actual_data__1,actual_data__2,error__drive__1,error__drive__2,error__signal__1,error__signal__2,dt_vec):
    ...
    axs[1].set_ylabel(r'$I_{di}$ [$\mu$A]')
    # axs[1].set_ylim([0,8])
    axs[1].set_xlim([0,np.round(target_data__drive[0,-1]*1e6/0.01)*0.01])
    axs[1].legend()
    # axs[1].set_title('Output signal in the DI loop (error = {:7.5e})'.format(error__signal))
...
def plot_wr_comparison__synapse(main_title,spike_times,target_drive,actual_drive,target_data,actual_data,wr_data_file_name,error_drive,error__si):
    ...
    axs[0].plot(actual_drive[0,:],actual_drive[1,:], '-', linewidth = pp['nominal_linewidth'], markersize = pp['nominal_markersize'], color = colors['green3'], label = 'soen_drive')
    axs[0].plot(target_drive[0,:],target_drive[1,:], '-', linewidth = pp['nominal_linewidth'], markersize = pp['nominal_markersize'], color = colors['yellow3'], label = 'wr_drive')  
    tn1 = np.min(target_drive[1,:])
    tn2 = np.max(target_drive[1,:])
    for ii in range(len(spike_times)):
        if ii == 0:
            axs[0].plot([spike_times[ii],spike_times[ii]],[tn1,tn2], '-.', color = colors['black'], linewidth = pp['fine_linewidth'], label = 'Spike times')             
        else:
            axs[0].plot([spike_times[ii],spike_times[ii]],[tn1,tn2], '-.', color = colors['black'], linewidth = pp['fine_linewidth'])             
    # axs[0].set_xlabel(r'Time [$\mu$s]')
    axs[0].set_ylabel(r'$I_{spd2}$ [$\mu$A]')
    axs[0].legend()
    # axs[0].set_title('Drive signal input SPD to Jsf (error = {:7.5e})'.format(error_drive))
     
    axs[1].plot(actual_data[0,:],actual_data[1,:], '-', linewidth = pp['nominal_linewidth'], markersize = pp['nominal_markersize'], color = colors['green3'], label = 'soen_sim')   
    axs[1].plot(target_data[0,:],target_data[1,:], '-', linewidth = pp['nominal_linewidth'], markersize = pp['nominal_markersize'], color = colors['yellow3'], label = 'WRSpice')             
    axs[1].set_xlabel(r'Time [ns]')
...
def plot_wr_comparison__synapse__n_fq_vs_I_de(M_vec,I_de_array,n_fq_1_array,n_fq_soen_array,num_jjs):
    
    fig = plt.figure()
    ax = fig.gca()
    fig.suptitle('num_jjs = {}\nM_big: n_fq_max = {:4.0f}; n_fq_min = {:4.0f}; n_fq_max/n_fq_min = {:4.0f}\nM_small: n_fq_max = {:4.0f}; n_fq_min = {:4.0f}; n_fq_max/n_fq_min = {:4.0f}'.format(num_jjs,n_fq_1_array[1][-1],n_fq_1_array[1][0],n_fq_1_array[1][-1]/n_fq_1_array[1][0],n_fq_1_array[0][-1],n_fq_1_array[0][0],n_fq_1_array[0][-1]/n_fq_1_array[0][0]))
    
    color_list_1 = ['red3','yellow3']
...
def plot__dend__error_vs_dt(dt_vec,error_array,error_drive_array,L_di_vec,tau_di_vec,title_str):
    ...
    fig, ax = plt.subplots(nrows = 1, ncols = 1)
    ax.loglog(dt_vec*1e9,error_drive_array[0,0,:], '-o', color = colors['grey8'], markersize = pp['nominal_markersize'], label = 'drive' )    
    for jj in range(len(L_di_vec)):        
        for kk in range(len(tau_di_vec)):
                    
            ax.loglog(dt_vec*1e9,error_array[jj,kk,:], '-o', color = colors[color_list[jj][kk]], markersize = pp['nominal_markersize'], label = 'L_di = {:7.2f}nH, tau_di = {:4.0f}ns'.format(L_di_vec[jj]*1e9,tau_di_vec[kk]*1e9)  )    
...
def plot_dend_rate_array__norm_to_phi0(**kwargs):
    ...
    # ax.set_ylabel('$\Phi_{in}/\Phi_0$',fontsize=24, fontweight='bold', labelpad=30) ; ax.set_ylim3d(influx_min-0.05,influx_max+0.05) #  [$\mu$A pH]
    ax.set_ylabel('$\Phi_{ex}/\Phi_0$', fontsize = 8, labelpad = 0)
    ax.set_ylim3d(0,1/2) #  [$\mu$A pH]
    # ax.set_ylim3d(0,0.4) #  [$\mu$A pH]
...
def plot__syn__wr_cmpr__single_pulse(soen_time,soen_drive,soen_response,wr_time,wr_drive,wr_response,L_di_vec,tau_di_vec,chi_drive,chi_signal,num_jjs):
...
def plot__syn__wr_cmpr__pulse_train(soen_time,soen_drive,soen_response,wr_time,wr_drive,wr_response,L_di_vec,I_de_vec,chi_drive,chi_signal,num_jjs):
...
def plot_phase_portrait(neuron_instance,**kwargs):
    ...
    refraction_vec = -neuron_instance.influx_vec__just_refraction/Phi_0 
    
    fig = plt.figure()    
    _str = ''
    if 'I_sy' in kwargs:
        _str = '{}I_sy = {:5.2f}uA'.format(_str,kwargs['I_sy'])
    if 'L_si' in kwargs:   
        _str = '{}; L_si = {:5.2f}nH'.format(_str,kwargs['L_si']*1e-3)
    if 'tau_si' in kwargs:   
        _str = '{}; tau_si = {:7.2f}us'.format(_str,kwargs['tau_si']*1e-3)
    if 'I_ne' in kwargs:   
        _str = '{}; I_ne = {:5.2f}uA'.format(_str,kwargs['I_ne'])
    fig.suptitle(_str) 
    ax = fig.gca()
    ...
    # ax.plot(drive_vec[neuron_instance.spike_time_indices],refraction_vec[neuron_instance.spike_time_indices], 'x', color = colors['red3'])
    ...
    ax.set_xlabel(r'$\Phi_{ex}^{nr}/\Phi_0$')
    ax.set_ylabel(r'$\Phi_{ref}^{nr}/\Phi_0$')
    # ax.legend()
        
    x_min = neuron_instance.dendrites['{}__d'.format(neuron_instance.name)].influx_list__dend[0]/Phi_0
