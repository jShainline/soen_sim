...
I_sy_vec = np.asarray([76,80]) # np.asarray([72,76,80])*1e-6 # [80e-6] # 
I_ne_vec = np.asarray([72,76,80]) # [72e-6] # 
tau_si_vec = np.asarray([500,1000,2000]) # [1e-6] # 
...
for ii in range(len(I_ne_vec)):
    ...
    for jj in range(len(I_sy_vec)):
        ...
        for kk in range(len(tau_si_vec)):
            ...
            file_name = file_format.format(I_sy,I_ne,tau_si)
            ...
            sy = synapse(name = 'sy',
                                synaptic_circuit_inductors = [100e3,100e3,250], # pH
                                synaptic_circuit_resistors = [5e6,4.005e3], # mOhm # [5e3,4.005],
