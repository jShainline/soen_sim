...
FileFormat = 'I_sy{:05.2f}uA_I_nf{:05.2f}uA_tau_si{:07.2f}ns_knrnf{:04.2f}'
for ii in range(len(I_nf_vec)):
    ...
    for jj in range(len(I_sy_vec)):
        ...
        for kk in range(len(tau_si_vec)):
            ...
            for mm in range(len(knrnf_vec)):
                knrnf = knrnf_vec[mm]
    
                print('ii = {} of {}; jj = {} of {}; kk = {} of {}; mm = {} of {}'.format(ii+1,len(I_nf_vec),jj+1,len(I_sy_vec),kk+1,len(tau_si_vec),mm+1,len(knrnf_vec)))
                    
                FilePrefix = FileFormat.format(I_sy,I_nf,tau_si,knrnf)
