...
# see pg 30-31 of green lab notebook started 20200514
...
Ib = 20 # uA
I_gate_on = 3 # uA, nTron gate threshold current
I_gate_off = 0.1*I_gate_on # uA, nTron gate reset current
I_channel_on = 0.8*Ib # uA, nTron channel activation current
I_channel_off = 0.2*I_channel_on # uA, nTron channel reset current
...
I_4_vec = np.zeros([_nt])
ii_vec = np.arange(1,_nt-1,1)
for ii in ii_vec:

    I_gate = -I_1_vec[ii]
    I_channel = Ib-I_4_vec[ii]
    ...
    ra = r1+r2
    rb = r3+r4        
    I_1_vec[ii+1] = (1-dt*(ra/La))*I_1_vec[ii] - (M/La)*( I_d[ii+1]-I_d[ii] ) - (L4/La)*( I_4_vec[ii]-I_4_vec[ii-1] )
    I_4_vec[ii+1] = (1-dt*(rb/Lb))*I_4_vec[ii] + dt*(r3/Lb)*Ib - (L4/Lb)*( I_1_vec[ii+1]-I_1_vec[ii] )
...
ax[2].plot(time_vec,I_4_vec, '-', color = colors['green3'])
