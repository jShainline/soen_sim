...
def synapse_time_stepper(time_vec,spike_times,num_jjs,L3,I_bias_list,tau_si):
    ...
    if len(spike_times) > 0:
        ...
        if num_jjs == 1:
            ...
            for ii in range(nt-1):
                ...
                I_drive = spd_current+I_bias_sy-I_c # -I_c in there because all data so far is with 40uA JJs and this is how I_drive is defined when making rate array
        ...
        if num_jjs == 3:
            ...
            for ii in range(nt-1):
                ...
                if spike_times[st_ind] <= _pt and _pt - spike_times[st_ind] < spd_duration:
                    ...
                    I_drive = I_bias_sy+spd_current-I_c
                    if I_drive < np.min(I_drive_list):
                        gf = 0
                    else:
                        I_drive_ind = (np.abs(I_drive_list[:] - I_drive)).argmin()
...
def synapse_current_distribution():
    
    I1 -> (-Lj2 (Ib2 L2 L3 + Ib3 L3 Lj3 + Ib2 (L2 + L3) Lj3) +

  Ib1 Lj1 (L3 (L2 + Lj2) + (L2 + L3 + Lj2) Lj3))/(

L2 L3 (L1 + Lj1) +

  L3 (L1 + L2 +

     Lj1) Lj2 + ((L2 + L3) (L1 + Lj1) + (L1 + L2 + L3 + Lj1) Lj2) Lj3)

 

I2 -> (-Ib3 L3 (L1 + Lj1 + Lj2) Lj3 + Ib1 Lj1 Lj2 (L3 + Lj3) +

  Ib2 (L1 + Lj1) Lj2 (L3 + Lj3))/(
...
I3 -> ((Ib3 L2 (L1 + Lj1) + (Ib1 Lj1 + Ib2 (L1 + Lj1) +

       Ib3 (L1 + L2 + Lj1)) Lj2) Lj3)/(
...
Ij1 -> (L3 (Ib1 L1 L2 + Ib2 L2 Lj2 +

     Ib1 (L1 + L2) Lj2) + (Ib1 L1 (L2 + L3) + (Ib3 L3 +

        Ib2 (L2 + L3) + Ib1 (L1 + L2 + L3)) Lj2) Lj3)/(
...
Ij2 -> (Ib3 L3 (L1 + Lj1) Lj3 + Ib1 Lj1 (L2 L3 + (L2 + L3) Lj3) +

  Ib2 (L1 + Lj1) (L3 Lj3 + L2 (L3 + Lj3)))/(
...
Ij3 -> (Ib3 L2 L3 (L1 + Lj1) +

  L3 (Ib1 Lj1 + Ib2 (L1 + Lj1) + Ib3 (L1 + L2 + Lj1)) Lj2)/(
