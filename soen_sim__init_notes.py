class synapse:
    synapse.unique_label = 's'+int
    synapse.neuronal_connections = [unique_label (input_neuron or input_signal), unique_label (output_neuron)] (label of neurons from which synapse receives and to which synapse connects)
    synapse.input_spike_times = list of real numbers (obtained from spike_times of neuronal_connection)
    synapse.loop_temporal_form = 'exponential' or 'power_law'
    synapse.time_constant = real (if 'exponential')
    synapse.power_law_exponent = real (-inf,0) (if 'power_law')
    synapse.integration_loop_inductance = real (L_si, henries)
    synapse.synaptic_bias_current = real (I_sy, amps)
    synapse.loop_bias_current = real (I_b, amps)
    synapse.loop_integrated_current = real function of time (I_si, amps; output variable)
