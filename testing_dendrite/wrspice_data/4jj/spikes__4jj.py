...
SpikeStart = np.loadtxt('SpikeStart__4jj.dat')
...
for iIde in np.arange(len(I_de_vector)):
    ...
    #     FilePrefix=FileFromat.format(Lleft=L_left_vector[0],LRight=L_left_vector[0],Ide=I_de_vector[iIde],Idrive=I_drive)
    #     if path.exists(rate.OutDatFile+FilePrefix+'.dat'):
